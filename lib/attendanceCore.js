...
// Fixed to IST regardless of the server's own timezone (e.g. Vercel runs in
// UTC) — otherwise check-in/check-out times drift with wherever this happens
// to be hosted.
...
function todayParts() {
  ...
  const time = now.toLocaleTimeString('en-GB', { timeZone: TIMEZONE }); // HH:MM:SS
  return { date, time };
}
